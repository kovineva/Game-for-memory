...
pygame.display.set_caption("Простая игра на память")

#размеры кругов и перемешка цветов
circle_radius = 50
...
font = pygame.font.SysFont('Time New Roman', 28)
pygame.display.update()

#время на запоминание
pygame.time.wait(5000)

#закрытие кругов, если цвета не совпали
for i in range(len(circle_pairs)):
    position = circle_positions[i]
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if len(uncovered_circles) == len(circle_pairs):
                final_score_text = font.render(f"Уровень вашей шикарной памяти: {str(score)} из 6", True, white)
